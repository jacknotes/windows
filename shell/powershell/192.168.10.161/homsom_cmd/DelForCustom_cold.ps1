...
function delfilelist($RemoteDir=$False)
{
    ...
    {
        Get-ChildItem -Path $RemoteDir -Recurse -ErrorAction SilentlyContinue -Filter *full.bak |Where-Object { $_.Extension -eq '.bak' }| Where-Object -FilterScript {($_.LastWriteTime -lt $day) -and ($_.PsISContainer -eq $False)} |
